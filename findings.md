...
# Phase 18 默认世界书合册与预设槽位精简调查

- “预设角色槽位”来自 SillyTavern `prompt_order` 的多角色顺序分组兼容层；雾灯谷当前只运行单角色会话，玩家无需手动切换，但导入/导出仍必须保存全部原始分组，不能删除底层数据。
- 四册系统默认世界书 ID 分别为 `mistvale-world-rules`、`mistvale-village-archive`、`mistvale-calendar-festivals`、`mistvale-production-partners`；角色卡与设置当前同时绑定四册。
- 合册应复用稳定的 `mistvale-world-rules` ID，避免引入第五个新 ID；其他三册作为旧版来源，只在版本迁移事务中合并后删除。
- 迁移必须按“世界规则 → 人物地点 → 岁时庆典 → 生产伙伴”固定顺序拼接现存条目，并以对象原值直接搬运，保留正文、关键词、次关键词、顺序、概率、位置和玩家修改。
- 玩家可能主动删除过某册默认世界书，因此迁移只能合并数据库中实际存在的默认册，不能借版本升级恢复已删除内容；任何其他自建世界书必须原样保留。
- 角色卡、会话和设置中的四册绑定需要统一替换为合册 ID并去重，同时保留所有非默认世界书绑定。
- 仓库发布包当前只额外携带“生产伙伴”一册而非全部四册；新默认改为单册后必须把该数组清空，否则新设备会再次出现第二册。六张伙伴卡仍引用四个旧 ID，也必须同步改为稳定主 ID，但内容包版本不应上调，以免旧设备覆盖玩家立绘。
- 现有版本 1–6 迁移会按阶段补回岁时册和生产册；版本 7 合册必须在这些旧迁移完成后运行，才能让真正的旧设备先补齐历史版本应有内容、再合成最终一册，而版本 6 用户删除的默认册不会被复活。
...
- 姓名链路现状：`GameState` 没有玩家档案，`GameAction` 没有改名动作，存档架构固定为 v1；`TavernSettings`/`ChatSession` 虽有 `userName`，但与游戏状态分离。因此第一阶段必须同时覆盖 GameState、v1→v2 存档、首登 UI 与设置改名，第二阶段再把酒馆请求改为逐次读取最新名字。
- 现有设置页已具备规则草稿、存档导入导出和新建游戏，可直接加入玩家档案区；顶栏为七列网格，桌面可在品牌区增加姓名副标签，手机端则隐藏副文案避免破坏 68px 高度。

# 2026-08-11 · Phase 20 酒馆契约加固

- 提供商根因：统一 `TavernApiConfig` 被所有协议直接复用，缺少能力矩阵；Cohere 因而收到两个互斥惩罚字段与负值，Anthropic/Gemini 则通过抽离全部 system 消息静默改变 SillyTavern 预设声明顺序。
- 资源根因：发送路径使用 `resolveSessionPreset` 与会话书单，显示路径却重新读取全局活动预设和角色默认书单，形成两个有效资源来源。
- 变量根因：导入 UI 只验证定义文件；模型 `<vars>` 在响应路径按任意 JSON 深度合并，且合并后的全局、会话、游戏镜像又整体写进会话，类型、范围与作用域均失效。
- 兼容根因：世界书和正则导入立即投影到内部子集，未保留原始 UID、未知字段和三态标志；导出无法重建未编辑的数据。正确边界是保存原始数据并覆盖受支持字段，而不是扩大内部运行时伪装宿主兼容。
- 精力根因：对话 `send` 在远程请求前不检查精力，并以组件生命周期 `settlementRef` 限制结算，导致零精力仍调用 API、同一窗口后续回合免费。
- 当前设计采用五个窄边界与两个内容所有权修复，不升级依赖、不更换 IndexedDB 表、不重写提示词编译器；旧记录通过可选兼容元数据继续读取。
- 提供商边界已加固：Cohere 使用独立采样范围与互斥惩罚校验；无法表达“历史之后 system 提示词”的 Anthropic、Gemini、Vertex Gemini 与 Cohere 会在发送前给出中文阻断错误，不再静默重排预设。
- 会话资源现在由 `resolveSessionResources` 唯一解析，发送路径、已挂载世界书计数和显示正则使用同一个会话绑定；删除被固定使用的预设时，会话与活动设置在同一 IndexedDB 事务内迁移到可用回退。
- 模型 `<vars>` 现在只允许更新已声明的全局或会话变量，先执行类型转换、数值范围限制、只读游戏镜像/未知键/歧义作用域拒绝，再把会话、全局定义和请求审计原子写入；拒绝与截断均进入诊断。
