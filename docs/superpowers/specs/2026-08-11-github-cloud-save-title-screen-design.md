...
- 开始界面固定提供“开始游戏”“读取存档”“创意工坊”“设置”四个按钮。
- 使用 ImageGen 内置的 ChatGPT Images 2 路径生成桌面横屏和手机竖屏两张精细像素封面。
...
- “读取存档”：打开双端存档中心，显示本地、云端和“导入 JSON”三种来源。
...
- 生成构图：2048×1152 横屏，精细 16-bit 农场像素插画，暮色山谷、农舍灯光、田垄、河流、矿山和轻魔法萤光。
- 主视觉为原创构图，只借鉴经典温馨农场模拟游戏的复古像素质感，不复制角色、Logo、建筑或地图。
- 标题必须逐字出现：`性撸谷物语`，位于画面上方中央，复古木牌/藤蔓/谷穗像素字标。
- 左下或下方中央为菜单安全区，背景细节降低以保证四按钮可读。

手机封面：

- 最终资产：`src/assets/xinglugu-title-mobile.webp`
- 生成构图：1024×1536 竖屏，保持相同地点、色彩和标题语言，纵向串联月亮、山谷、农舍和前景花草。
- 上部标题安全区、下部四按钮安全区必须避开人物和高对比细节。

两张图均由内置 ImageGen 分别生成并人工检查标题准确性。若模型文字不准确，先进行一次定向编辑；仍不准确时保留无文字场景，并由前端像素字标精确覆盖，同时提供语义化 `<h1>`，不能发布错别字封面。

### 3.3 UI 结构

- 使用语义化 `<main>`、`<h1>` 和四个 `<button>`；唯一 ID：`title-start-game`、`title-load-save`、`title-workshop`、`title-settings`。
