...
import { TAVERN_TITLE } from '../../branding'
...
const ImageGenerationPanel = lazy(() => import('./panels/ImageGenerationPanel').then((module) => ({ default: module.ImageGenerationPanel })))
...
const tabs = [
  ...
  { id: 'images', label: '绘图', note: '对话生图', icon: 'image' },
...
export function TavernHubModal({ onClose, initialTab = 'api' }: { onClose(): void; initialTab?: TabId }) {
  ...
  return <section className="tavern-hub" aria-labelledby="tavern-hub-title">
    <header className="tavern-hub-header"><div className="tavern-hub-sigil" aria-hidden="true"><span>T</span><i /><i /><i /></div><div><p>SILLYTAVERN STORY CONSOLE</p><h2 id="tavern-hub-title">{TAVERN_TITLE}</h2><span>模型连接、世界信息、角色记忆与剧情楼层控制台</span></div><div className="tavern-hub-state"><i data-state={tavern.status} /><span>{tavern.status === 'ready' ? '酒馆档案已就绪' : tavern.status === 'loading' ? '正在读取酒馆档案' : '酒馆档案异常'}</span><strong>LLM API REQUIRED</strong></div><button id="tavern-hub-close" className="icon-button" type="button" aria-label="关闭酒馆中枢" onClick={onClose}><GameIcon name="close" size={18} /></button></header>
