...
export function WorkshopHub({ account, onOpenStudio }: WorkshopHubProps) {
  ...
  return <section className={`community-workshop ${selected ? 'has-detail' : ''}`} aria-labelledby="start-workshop-title">
    <div className="community-workshop-nav" role="tablist" aria-label="创意工坊页面">
      ...
      <div className="community-workshop-account">{account === null ? <span>正在确认 GitHub 身份</span> : authenticated ? <><img src={account.user.avatarUrl} alt="" referrerPolicy="no-referrer" /><span>{account.user.login}<small>已连接 GitHub</small></span></> : account.configured ? <a id="workshop-github-login" href={loginUrl}><GameIcon name="connect" size={17} />登录 GitHub</a> : <span>GitHub 登录尚未配置</span>}</div>
    </div>
...
function WorkshopPublisher({ account, editing, source, loginUrl, busy, onBusy, onError, onPublished }: PublisherProps) {
  ...
  return <section className="community-workshop-section"><header><span>PUBLISHING DESK</span><h2>{editing ? `更新「${editing.item.title}」` : '发布玩家资源'}</h2><p>发布前会移除会话、存档、密钥和全局变量；公开立绘会被打包为跨设备图片。</p></header>{account?.authenticated !== true && <div className="community-workshop-login is-inline"><span>{account?.configured === false ? '部署端尚未配置 GitHub OAuth，暂时只能编辑本地作品。' : '发布、更新和撤回需要 GitHub 身份。'}</span>{account?.configured !== false && <a href={loginUrl}>登录 GitHub</a>}</div>}<form className="community-workshop-publisher" onSubmit={(event) => void submit(event)}>
